...
class RenderSettings:
    ...
    # Output gamma curve for IPP2 display-referred output.
    # For monitoring parity with camera (IPP2 / BT.709 / BT.1886), set to GAMMA_BT1886 (32).
    # This applies the display gamma transform to produce correct contrast.
    # Without this, IPP2 output may appear flat/gray (missing gamma encoding).
    output_gamma_curve: Optional[int] = None
...
def build_redline_command(
    ...
    # Output gamma curve for IPP2 display-referred rendering.
    # This is separate from the creative gammaCurve and applies the final
    # display gamma (e.g., BT.1886) to produce correct contrast in the output.
...
def make_contact_sheet_job(
    ...
    *,
    ...
    For IPP2 monitoring parity (Medium Contrast / Medium Highlight Rolloff),
    set output_gamma_curve=GAMMA_BT1886 to apply BT.1886 display gamma.
    """
...
def _validate_settings(settings: RenderSettings) -> None:
    ...
    if settings.use_meta and (
        settings.gamma_curve is not None or settings.color_space is not None
    ):
        raise ValueError(
            "Cannot combine use_meta=True with manual gammaCurve/colorSpace overrides. "
            "Either use metadata-driven color or set use_meta=False and provide overrides."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-gamma-curve", type=int, default=None, help="Output gamma curve for IPP2 display transform (32=BT.1886)")
